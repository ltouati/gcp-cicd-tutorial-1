        ...
        withMaven(
            ...
            mavenLocalRepo: '.repository') {
                ...
                gcloud(serviceAccountCredential: 'gcp-instance-creator') {
                    ...
                    sh "gcloud beta compute instances create-with-container test-${env.BUILD_ID}-${commit_id} --zone=${default_zone} --scopes=https://www.googleapis.com/auth/devstorage.read_only,https://www.googleapis.com/auth/logging.write,https://www.googleapis.com/auth/monitoring.write,https://www.googleapis.com/auth/servicecontrol,https://www.googleapis.com/auth/service.management.readonly,https://www.googleapis.com/auth/trace.append --machine-type=n1-standard-1  --image-project=cos-cloud --boot-disk-size=10GB --boot-disk-type=pd-standard --container-image=eu.gcr.io/${project_id}/todo-app:${env.BUILD_ID}-${commit_id} --container-restart-policy=always --preemptible"
                }
        }
    }
}